  ...
  : describe("Lottery", function () {
      ...
      describe("constructor", function () {
        it("initializes the Lottery correctly", async function () {
          ...
          expect(entranceFee).to.equal(networkConfig[chainId].entranceFee)
      ...
      describe("enterLottery", function () {
        describe("happy path", function () {
          it("adds the correct player in the players array and adds the correct value to the contract", async function () {
            const playersNber_Initial = (await lottery.getPlayers()).length
            await lottery.connect(user01).enterLottery({ value: entranceFee })
        ...
        describe("unhappy path", function () {
          it("reverts if the lottery is not opened", async function () {
            ...
            // assert
            await expect(
              lottery.connect(user01).enterLottery({ value: entranceFee })
      ...
      describe("checkUpkeep", function () {
        ...
        it("returns false if enough time has not passed", async () => {
          ...
          await network.provider.send("evm_mine", [])
          const { upkeepNeeded } = await lottery.callStatic.checkUpkeep([]) // upkeepNeeded = (isOpen && timePassed && hasPlayer && isFunded)
          expect(!upkeepNeeded)
        })
      ...
      describe("performUpkeep", function () {
        describe("happy path", function () {
          it("can only run if upkeepNeeded is TRUE", async function () {
            ...
            await network.provider.send("evm_mine", [])
            let { upkeepNeeded } = await lottery.callStatic.checkUpkeep([])
            expect(upkeepNeeded)
          ...
          it("updates the lottery state", async function () {
            ...
            let requestId = transactionReceipt.events[1].args.requestId
      ...
      describe("fulfillRandomWords", function () {
        ...
        it("sets the winner, reset the lottery, and sends value to winner", async function () {
          ...
          const deployer_Balance_Initial = await ethers.provider.getBalance(
            deployer.address
          )
          ...
          for (let i = 1; i < 4; i++) {
            ...
            let player_Balance_Initial = await ethers.provider.getBalance(
              player.address
            )
            ...
            lottery.once("WinnerPicked", async () => {
              ...
              try {
                ...
                expect(winner_Balance).to.equal(
                  players_Balances_Initial[indexOfWinner].add(prize)
                )
